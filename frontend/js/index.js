...
async function loadHomeStats() {
    try {
        ...
        if (totalPlayers) {
            totalPlayers.textContent = stats.totalUsers?.toLocaleString() || '0';
        }
        
        const totalPoints = document.getElementById('heroTotalPoints');
        if (totalPoints) {
            totalPoints.textContent = stats.totalPoints?.toLocaleString() || '0';
        }
        
        const gamesToday = document.getElementById('heroGamesToday');
        if (gamesToday) {
            // Use total games for now (can be enhanced to track today's games)
            gamesToday.textContent = stats.totalGames?.toLocaleString() || '0';
        }
    } catch (error) {
...
async function loadChallenges() {
    try {
        ...
        // Update Play Any Game Challenge
        ...
        // Update Weekly Streak Challenge
